...
namespace App\Tests\State\News;
...
class NewsProviderTest extends KernelTestCase
{
    ...
    {
        $this->client->get(Argument::cetera())->shouldBeCalledOnce()->willReturn($this->getData());

        $operation = $this->prophesize(Operation::class);
        $operation->getName()->willReturn("api_collection");
        $operation->getPaginationItemsPerPage()->willReturn(30);
        $operation->getPaginationClientItemsPerPage()->willReturn(30);
        $operation->getPaginationMaximumItemsPerPage()->willReturn(100);

        $provider = $this->getProvider();

        $data = $provider->provide($operation->reveal());

        $this->assertInstanceOf(Paginator::class, $data);

        $this->assertCount(30, $data->getIterator());

        $news = $data->getIterator()->current();

        $this->assertInstanceOf(News::class, $news);

        $this->assertEquals("BBC News", $news->source);
        $this->assertEquals("https://www.facebook.com/bbcnews", $news->author);
    }


    public function testProvideItemWhenCached(): void
    {
        $operation = $this->prophesize(Operation::class);
        ...
        $this->cache->get(Argument::cetera())->willReturn($news->reveal());

        $provider = $this->getProvider();
        $data = $provider->provide($operation->reveal(), ['id' => $id]);

        $this->assertEquals($data, $news->reveal());
    }

    public function testProvideItemWhenNotCached(): void
    {
        $operation = $this->prophesize(Operation::class);
        $operation->getName()->willReturn("api_item");
        ...
        $this->expectExceptionMessage("news.not_found");

        $provider->provide($operation->reveal(), ['id' => $id]);
    }


    private function getProvider(): NewsProvider
    {
        return new NewsProvider(
            $this->client->reveal(),
            self::getContainer()->get(Pagination::class),
            $this->cache->reveal(),
        );
    }
